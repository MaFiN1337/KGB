...
def extract_names(
    text: str, client: "_anthropic.Anthropic", model: str, usage: dict
) -> list[str]:
    try:
...
def call_llm(
    ...
    names_line = (
        "Відомі особи у тексті (лише для довідки, НЕ джерело цитат): "
        + ", ".join(known_names) + "\n\n"
    ) if known_names else ""
    try:
        resp = client.messages.create(
            ...
            messages=[{"role": "user", "content": speaker_line + names_line + text[:3500]}],
        )
        usage["in"] += resp.usage.input_tokens
        usage["out"] += resp.usage.output_tokens
        data = _extract_json(resp.content[0].text)
...
def main():
    parser = argparse.ArgumentParser(description="KGB archive NLP entity extractor — Claude version")
    ...
    for i, doc in enumerate(documents, 1):
        ...
        known_names = extract_names(text, client, args.model, usage)
        print(f"names={known_names}", end=" | ", flush=True)

        result = call_llm(text, current_speaker, client, args.model, usage, known_names)
    ...
    edges_raw_path = Path(args.output_dir) / "edges_raw_claude.csv"
    ...
    normalize_cmd = [
        sys.executable,
        str(Path(__file__).parent / "normalize_names_claude.py"),
